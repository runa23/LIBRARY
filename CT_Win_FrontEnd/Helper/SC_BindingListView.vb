...
Public Interface SC_ICanFixSearch
    Sub setFixSearch(ByVal plFixSearch As Boolean)
    Function getFixSearch() As Boolean
End Interface

Public Class SC_BindingListView(Of T)
    Inherits BindingList(Of T)
    Implements IBindingListView
    Implements IRaiseItemChangedEvents
    Implements SC_ICanFixSearch
    ...
    Private l_fixSearch As Boolean = False
    ...
    Protected Overloads Overrides Function FindCore(ByVal [property] As PropertyDescriptor, ByVal key As Object) As Integer
        For i As Integer = 0 To Count - 1
            ' Simple iteration:
            Dim item As T = Me(i)
            If TypeOf ([property].GetValue(item)) Is String Then
                If l_fixSearch Then
                    If CStr([property].GetValue(item)).ToLower.Trim.Equals(key.ToString.ToLower.Trim) Then
                        Return i
                    End If
                Else
                    If CStr([property].GetValue(item)).ToLower.Trim.StartsWith(key.ToString.ToLower.Trim) Then
                        Return i
                    End If
                End If
...
#End Region
    ...
    Public Sub setFixSearch(ByVal plFixSearch As Boolean) Implements SC_ICanFixSearch.setFixSearch
        l_fixSearch = plFixSearch
    End Sub

    Public Function getFixSearch() As Boolean Implements SC_ICanFixSearch.getFixSearch
        Return l_fixSearch
    End Function
End Class
